    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="门店名称" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input placeholder="请输入门店名称" v-decorator.trim="[ 'storeAddr', validatorRules.storeAddr]" :readOnly="!!model.id"/>
        </a-form-item>

        <template>
          <a-form-item label="门店地址" :labelCol="labelCol" :wrapperCol="wrapperCol" >
            <a-input  placeholder="请输入门店地址" v-decorator="[ 'storeName',validatorRules.storeName]" />
          </a-form-item>
        </template>
      </a-form>
    </a-spin>
  ...
  export default {
    ...
    methods: {
      ...
      edit (record) {
        ...
        that.$nextTick(() => {
          that.form.setFieldsValue(pick(this.model,'storeName','storeAddr','post'))
        });
        //身份为上级显示负责部门，否则不显示
      ...
      handleSubmit () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            if(!this.model.id){
              formData.id = this.id;
              obj=addStore(formData);
            }else{
              obj=editStore(formData);
            }
            obj.then((res)=>{
              if(res.success){
