    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="公司名称" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input placeholder="请输入公司名称" v-decorator="['name', validatorRules.name]"/>
        </a-form-item>
        ...
        <a-form-item label="有效时间" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-date-picker
            ...
            style="width: 100%"
            placeholder="请选择有效时间"
            v-decorator="['validTime', {initialValue:!model.validTime?null:moment(model.validTime,dateFormat)}]"
...
<script>
  ...
  import {addKeyRule,editKeyRule,queryUserRole,queryall} from '@/api/api'
      ...
      handleSubmit () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            if (values.validTime != null){
              values.validTime = values.validTime.format(this.dateFormat);
            }
            ...
            if(!this.model.id){
              ...
              obj=addKeyRule(formData);
            }else{
              obj=editKeyRule(formData);
            }
            obj.then((res)=>{
              if(res.success){
