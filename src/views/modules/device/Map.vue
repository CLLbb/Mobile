    ...
    <a-col :md="20" :sm="24">
      <a-card :bordered="false">
        <baidu-map class="map" :center="map.center" :zoom="map.zoom" @ready="handler">
          ...
          <bm-geolocation anchor="BMAP_ANCHOR_BOTTOM_RIGHT" :showAddressBar="true" :autoLocation="true"></bm-geolocation>
          <!--点-->
          <div v-for="item in map.markers">
<!--          <bm-marker  v-for="item in map.markers" :position="{lng: item.lng, lat: item.lat}" :dragging="map.dragging" animation="BMAP_ANIMATION_DROP" @click="lookDetail(item)">-->
<!--            <bm-marker v-if="item.status &gt; 2" :position="{lng: item.lng, lat: item.lat}" :dragging="map.dragging" animation="BMAP_ANIMATION_DROP" @click="lookDetail(item)" :icon="{url: marderURL+'/marker2.png', size: {width: 50, height: 50}}">-->
<!--              &lt;!&ndash;提示信息&ndash;&gt;-->
...
<!--              </bm-info-window>-->
<!--            </bm-marker>-->
<!--            <bm-marker v-if="item.status &lt; 3" :position="{lng: item.lng, lat: item.lat}" :dragging="map.dragging" animation="BMAP_ANIMATION_DROP" @click="lookDetail(item)" :icon="{url: marderURL+'/marker1.png', size: {width: 50, height: 50}}">-->
<!--              &lt;!&ndash;提示信息&ndash;&gt;-->
<!--              <bm-info-window :title="map.infoWindow.info.title" :position="{lng: map.infoWindow.info.lng, lat: map.infoWindow.info.lat}"   :show="item.showFlag" @close="infoWindowClose(item)" @open="infoWindowOpen(item)">-->
<!--              </bm-info-window>-->
<!--            </bm-marker>-->
            <bm-marker v-if="item.status == 3" :position="{lng: item.lng, lat: item.lat}" :dragging="map.dragging" animation="BMAP_ANIMATION_DROP" @click="lookDetail(item)">
              <!--提示信息-->
              <bm-info-window :title="map.infoWindow.info.title" :position="{lng: map.infoWindow.info.lng, lat: map.infoWindow.info.lat}"   :show="item.showFlag" @close="infoWindowClose(item)" @open="infoWindowOpen(item)">
              </bm-info-window>
            </bm-marker>
...
<script>
  ...
  export default {
    ...
    created(){
      let that = this;
      that.marderURL = window._CONFIG['markerURL'];
    },
    methods: {
      handler ({BMap, map}) {
        let that = this;
      ...
      onSelect(selectedKeys, e) {
        ...
        queryDeviceById(param).then((res) => {
          if (res.success && res.result ) {
            that.map.center = {lng: res.result.lng, lat: res.result.lat,status: res.status};
          }
        })
      },
    }
